...
// Get or create the combination presets data file in Drive
async function getOrCreateCombinationPresetsFile() {
    if (!isSignedIn || !formsFolderId) {
        console.log('Cannot sync combination presets: not signed in or no forms folder');
        return null;
    }

    try {
        const PRESETS_FILE_NAME = 'Combination_Presets.json';

        // Search for existing presets file
        const searchResponse = await gapi.client.drive.files.list({
            q: `name='${PRESETS_FILE_NAME}' and '${formsFolderId}' in parents and trashed=false`,
            spaces: 'drive',
            fields: 'files(id, name, modifiedTime)'
        });

        if (searchResponse.result.files.length > 0) {
            combinationPresetsFileId = searchResponse.result.files[0].id;
            console.log('Found existing combination presets file:', combinationPresetsFileId);
            return combinationPresetsFileId;
        }

        // Create new presets file
        console.log('Creating new combination presets file...');
        const createResponse = await gapi.client.drive.files.create({
            resource: {
                name: PRESETS_FILE_NAME,
                mimeType: 'application/json',
                parents: [formsFolderId]
            },
            fields: 'id'
        ...
        combinationPresetsFileId = createResponse.result.id;
        console.log('Created combination presets file:', combinationPresetsFileId);

        // Initialize with empty object
        await updateCombinationPresetsFile({});

        return combinationPresetsFileId;
    } catch (error) {
        console.error('Error with combination presets file:', error);
        return null;
    }
}

// Update the combination presets file content in Drive
async function updateCombinationPresetsFile(data) {
    if (!combinationPresetsFileId) {
        console.error('No combination presets file ID');
        return false;
    }
    ...
    const response = await fetch(
        `https://www.googleapis.com/upload/drive/v3/files/${combinationPresetsFileId}?uploadType=multipart`,
        {
            method: 'PATCH',
            headers: {
                'Authorization': 'Bearer ' + gapi.client.getToken().access_token,
    ...
    if (!response.ok) {
        throw new Error('Failed to update combination presets file');
    }
