...
// ========== COMBINED TEMPLATES MODAL ==========

// Open combined templates modal
async function openTemplatesModal() {
    ...
    // Check if signed in
    if (!isSignedIn) {
        document.getElementById('formsNotConnected2').style.display = 'block';
        document.getElementById('formsConnected2').style.display = 'none';
        document.getElementById('excelNotConnected2').style.display = 'block';
        document.getElementById('excelConnected2').style.display = 'none';
    } else {
        document.getElementById('formsNotConnected2').style.display = 'none';
        document.getElementById('formsConnected2').style.display = 'block';
        document.getElementById('excelNotConnected2').style.display = 'none';
        document.getElementById('excelConnected2').style.display = 'block';

        // Ensure folders exist and load data
        await getOrCreateFormsFolder();
        await loadFormTemplates();
        displayFormsList2();
...
// Close combined templates modal
function closeTemplatesModal() {
    document.getElementById('templatesModal').classList.remove('active');
    document.getElementById('formFileInput2').value = '';
    document.getElementById('excelTemplateFile2').value = '';
    document.getElementById('excelTemplateName2').value = '';
}

// Display forms list in new modal
function displayFormsList2() {
    const container = document.getElementById('formsListContainer2');

    if (!formTemplates || Object.keys(formTemplates).length === 0) {
        container.innerHTML = '<div style="text-align: center; padding: 15px; color: #7f8c8d; font-size: 13px;">No forms uploaded yet</div>';
        return;
    }
    ...
    for (const [formType, formData] of Object.entries(formTemplates)) {
        const formName = formTypeNames[formType] || formType.replace(/_/g, ' ').toUpperCase();
        ...
        html += `
            <div class="file-item" style="margin-bottom: 10px; padding: 10px; background: white; border: 1px solid #ecf0f1; border-radius: 6px;">
                <div class="file-info">
                    <span style="font-size: 18px;">${formData.fileType === 'application/pdf' ? '📄' : '🖼️'}</span>
                    <div class="file-details">
                        <h4 style="font-size: 13px; margin-bottom: 3px;">${formName}</h4>
                        <p style="font-size: 11px; color: #7f8c8d;">${formData.fileType || 'Unknown type'}</p>
                        ${hasFieldMapping ? '<p style="font-size: 11px; color: #27ae60; margin-top: 2px;">✓ ' + fieldCount + ' field(s) mapped</p>' : ''}
                    </div>
                </div>
                <div class="file-actions">
                    ${hasFieldMapping ? '<button class="btn btn-small" style="font-size: 11px; padding: 4px 8px;" onclick="openFieldMappingModal(\'' + formType + '\')">⚙️ Configure</button>' : ''}
                    <button class="btn btn-small" style="font-size: 11px; padding: 4px 8px;" onclick="viewForm('${formType}')">View</button>
                    <button class="btn btn-danger btn-small" style="font-size: 11px; padding: 4px 8px;" onclick="deleteForm('${formType}')">Delete</button>
                </div>
            </div>
        `;
    }
...
// Upload form template from new modal
async function uploadFormTemplateNew() {
    const fileInput = document.getElementById('formFileInput2');
    const formTypeSelect = document.getElementById('formTypeSelect2');
    ...
    const oldFileInput = document.getElementById('formFileInput');
    const oldFormTypeSelect = document.getElementById('formTypeSelect');

    // Temporarily transfer values
    const dataTransfer = new DataTransfer();
    dataTransfer.items.add(file);
    oldFileInput.files = dataTransfer.files;
    oldFormTypeSelect.value = formType;

    // Call existing function
    await uploadFormTemplate();

    // Refresh the list in new modal
    displayFormsList2();
    fileInput.value = '';
}

// Load Excel templates in new modal
        ...
        html += `
            <div class="file-item" style="margin-bottom: 10px; padding: 10px; background: white; border: 1px solid #ecf0f1; border-radius: 6px;">
                <div class="file-info">
                    ...
                    <div class="file-details">
                        <h4 style="font-size: 13px; margin-bottom: 3px;">${template.name}</h4>
                        <p style="font-size: 11px; color: #7f8c8d;">${Object.keys(template.mappings || {}).length} fields mapped</p>
                    </div>
                </div>
                <div class="file-actions">
...
// Create Excel template from new modal
async function createExcelTemplateNew() {
    const nameInput = document.getElementById('excelTemplateName2');
    const fileInput = document.getElementById('excelTemplateFile2');

    // Transfer to old inputs
    const oldNameInput = document.getElementById('excelTemplateName');
    const oldFileInput = document.getElementById('excelTemplateFile');
