...
// ============ Combination Presets Management ============

// Load combination presets from Google Drive and localStorage
async function loadCombinationPresets() {
    // Try to load from Google Drive first
    if (isSignedIn && formsFolderId) {
        try {
            await getOrCreateCombinationPresetsFile();

            if (combinationPresetsFileId) {
                const response = await gapi.client.drive.files.get({
                    fileId: combinationPresetsFileId,
                    alt: 'media'
                });

                if (response.result && Object.keys(response.result).length > 0) {
                    formCombinationPresets = response.result;
                    // Cache in localStorage
                    localStorage.setItem('formCombinationPresets', JSON.stringify(formCombinationPresets));
                    console.log('Loaded combination presets from Drive:', Object.keys(formCombinationPresets).length);
                    return;
                }
            }
        } catch (error) {
            console.error('Error loading combination presets from Drive:', error);
        }
    }

    // Fallback to localStorage
    const stored = localStorage.getItem('formCombinationPresets');
    if (stored) {
        try {
            formCombinationPresets = JSON.parse(stored);
            console.log('Loaded combination presets from localStorage:', Object.keys(formCombinationPresets).length);
        } catch (e) {
            console.error('Error parsing stored combination presets:', e);
            formCombinationPresets = {};
        }
    }
}

// Save combination presets to Google Drive and localStorage
async function saveCombinationPresets() {
    // Save to localStorage for quick access
    localStorage.setItem('formCombinationPresets', JSON.stringify(formCombinationPresets));
    ...
    if (isSignedIn && formsFolderId) {
        try {
            ...
            if (combinationPresetsFileId) {
                await updateCombinationPresetsFile(formCombinationPresets);
                console.log('Saved combination presets to Drive');
            }
        } catch (error) {
            console.error('Error saving combination presets to Drive:', error);
        }
    }
}
...
async function createCombinationPreset(name, side1, side2) {
    if (!name || !side1 || !side2) {
        alert('Please provide a preset name and select both forms');
        return false;
    }
    ...
    // Create preset
    formCombinationPresets[presetId] = {
        id: presetId,
        name: name,
        side1: side1,
        side2: side2,
    ...
    // Save to storage
    await saveCombinationPresets();
    console.log('Created combination preset:', name);

    return true;
}

// Delete a combination preset
async function deleteCombinationPreset(presetId) {
    if (!formCombinationPresets[presetId]) {
        alert('Preset not found');
        return false;
    }

    const presetName = formCombinationPresets[presetId].name;

    if (confirm(`Are you sure you want to delete the preset "${presetName}"?`)) {
        delete formCombinationPresets[presetId];
        await saveCombinationPresets();
        console.log('Deleted combination preset:', presetName);
        return true;
    }
...
// Use a preset to combine and print forms
async function useCombinationPreset(presetId, customerId) {
    const preset = formCombinationPresets[presetId];
    ...
    try {
        ...
        // Render forms with customer data
        const form1Data = await renderFormWithData(preset.side1, customer);
        const form2Data = await renderFormWithData(preset.side2, customer);
        ...
        const form1Name = formTypeNames[preset.side1] || preset.side1;
        const form2Name = formTypeNames[preset.side2] || preset.side2;
        ...
        printWin.document.write(`
            ...
            <head>
                <meta charset="UTF-8">
                <title>${customer.name} - ${preset.name}</title>
                <style>
                    @page {
                        size: A4;
                        margin: 0;
            ...
            <body>
                <button class="print-btn no-print" onclick="window.print()">Print Double-Sided</button>
                <div class="info-banner no-print">
                    <strong>${customer.name}</strong><br>
                    Preset: ${preset.name} | Page 1: ${form1Name} | Page 2: ${form2Name}
                </div>
        ...
        alert('Failed to print using preset: ' + error.message);
    }
}
